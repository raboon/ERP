...
namespace Gastro
{
    ...
    public class FileData
    {
        ...
        public FileData(string dir, string name, System.IO.FileAttributes attributes, long size, DateTime createdUTC, DateTime modifiedUTC, DateTime accessedUTC)
        {
            this.Path = System.IO.Path.Combine(dir, name);
            this.Name = name;
            this.Attributes = attributes;
            this.Size = size;
            this.CreationTimeUtc = createdUTC;
            this.LastWriteTimeUtc = modifiedUTC;
            this.LastAccessTimeUtc = accessedUTC;
        }

        public FileData(DirectoryInfo dirInfo)
        {
            this.Path = dirInfo.FullName;
            this.Name = dirInfo.Name;
            this.Attributes = dirInfo.Attributes;
            this.Size = 0;
            this.CreationTimeUtc = dirInfo.CreationTimeUtc;
            this.LastWriteTimeUtc = dirInfo.LastWriteTimeUtc;
            this.LastAccessTimeUtc = dirInfo.LastAccessTimeUtc;
        }
    ...
    public static class FastDirectoryEnumerator
    {
        ...
        {
            return FastDirectoryEnumerator.EnumerateFiles(path, searchPattern, System.IO.SearchOption.TopDirectoryOnly);
        }
        ...
        /// </exception>
        public static IEnumerable<FileData> EnumerateFiles(string path, string searchPattern, System.IO.SearchOption searchOption)
        {
            if (path == null)
            {
            ...
            if ((searchOption != System.IO.SearchOption.TopDirectoryOnly) && (searchOption != System.IO.SearchOption.AllDirectories))
            {
        ...
        /// <paramref name="filter"/> is a null reference (Nothing in VB)
        /// </exception>
        public static FileData[] GetFiles(string path, string searchPattern, System.IO.SearchOption searchOption)
        {
        ...
        private class FileEnumerable : IEnumerable<FileData>
        {
            ...
            private readonly System.IO.SearchOption m_searchOption;
            ...
            /// operation should include all subdirectories or only the current directory.
            /// </param>
            public FileEnumerable(string path, string filter, System.IO.SearchOption searchOption)
            {
                m_path = path;
                m_filter = filter;
        ...
        private class FileEnumerator : IEnumerator<FileData>
        {
            ...
            {
                public readonly string Path;
                public Stack<string> SubdirectoriesToProcess = new Stack<string>();
                public int currPhase = 0;       // 0 = findfirst; 1 = findnext; 2 = process subdirs
            ...
            /// operation should include all subdirectories or only the current directory.
            /// </param>
            public FileEnumerator(string path, string filter, System.IO.SearchOption searchOption)
            {
                m_path = path;
                m_filter = filter;
                m_searchOption = searchOption;
                ...
                if (m_searchOption == System.IO.SearchOption.AllDirectories)
                {
            ...
            public bool MoveNext()
            {
                ...
                if (m_currentContext.currPhase != 2)
                {
                    if (m_hndFindFile == null)
                    {
                ...
                m_currentContext.currPhase = retval ? 1 : 2;
                ...
                if (retval)
                {
                    ...
                    // save subdirectory to scan later
                    if (m_searchOption == System.IO.SearchOption.AllDirectories && ((System.IO.FileAttributes)m_win_find_data.dwFileAttributes & System.IO.FileAttributes.Directory) == System.IO.FileAttributes.Directory)
                        m_currentContext.SubdirectoriesToProcess.Push(m_path);
                }
                else if (m_searchOption == System.IO.SearchOption.AllDirectories)
                {
